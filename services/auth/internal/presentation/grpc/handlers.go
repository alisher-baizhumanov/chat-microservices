...
func (s *userServer) Update(ctx context.Context, updateIn *desc.UpdateIn) (*emptypb.Empty, error) {
	...
	slog.InfoContext(ctx, "update user",
		slog.Int64("id", updateIn.GetId()),
		slog.String("role", updateArgs.GetRole().String()),
		slog.String("name", updateArgs.GetName().String()),
		slog.String("email", updateArgs.GetEmail().String()),
	)
